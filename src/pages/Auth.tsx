...
export default function Auth() {
  ...
  if (user) { void navigate(next, { replace: true }); return null; }
  const requestCode = async (event: FormEvent) => {
    ...
    setBusy(false); setSent(!error); setMessage(error ? error.message : "We sent a six-digit code to your email.");
  };
  const verifyCode = async (event: FormEvent) => {
    event.preventDefault(); if (!supabase) return;
    setBusy(true); const { error } = await supabase.auth.verifyOtp({ email, token: code, type: "email" });
    setBusy(false); setMessage(error ? error.message : "Signed in. Redirecting…");
    if (!error) navigate(next, { replace: true });
  };
